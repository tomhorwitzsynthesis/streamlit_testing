...
def render(source: str = "pr"):
    try:
        affinity_data = load_audience_affinity_outputs(source=source)
        ...
        view_option = st.selectbox(
            ...
            key=f"affinity_view_{(source or 'pr').lower()}"
        )

        # Title context for source
        source_label = "Press Releases" if (source or "pr").lower() == "pr" else "LinkedIn"
        st.subheader(f"🔍 {view_option} View — {source_label}")
