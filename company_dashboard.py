...
def generate_company_dashboard(input_file):
    """
    Generates a Streamlit dashboard for a given company's data file.
    ...
    output_file = f"{os.path.splitext(input_file)[0]}_processed.xlsx"
    ...
    company_name = os.path.splitext(os.path.basename(input_file))[0].replace("_data", "").replace("_processed", "").title()
    title = f"{company_name} Dashboard"
    ...
    matrix_image_path = os.path.join(script_dir, "matrix.jpg")
